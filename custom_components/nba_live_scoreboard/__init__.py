...
from .coordinator import NbaLiveScoreboardCoordinator

_LOGGER = logging.getLogger(__name__)

CONFIG_SCHEMA = cv.config_entry_only_config_schema(DOMAIN)

type RuntimeData = NbaLiveScoreboardCoordinator
...
CARD_FILENAME = "nba-live-game-card.js"
...
async def _async_register_card(
    ...
    try:
        ...
        if existing:
            for res in existing:
                if res.get("url") != card_url or res.get("type") != "module":
                    _LOGGER.info(
                        "Updating NBA Live Game Card resource: %s -> %s",
                        res.get("url"),
                        card_url,
                    )
                    ...
                    _LOGGER.debug("NBA Live Game Card already registered with current URL")
            return
        ...
        _LOGGER.info("Registered NBA Live Game Card as Lovelace resource: %s", card_url)
    except ImportError:
...
def _any_coordinator(hass: HomeAssistant) -> NbaLiveScoreboardCoordinator | None:
    ...
    for value in (hass.data.get(DOMAIN) or {}).values():
        if isinstance(value, NbaLiveScoreboardCoordinator):
            return value
    return None


def _coordinator_for_entity(hass: HomeAssistant, entity_id: str) -> NbaLiveScoreboardCoordinator | None:
    ...
    if record is not None and record.config_entry_id:
        ...
        if isinstance(candidate, NbaLiveScoreboardCoordinator):
            return candidate

    coordinators = [v for v in domain_data.values() if isinstance(v, NbaLiveScoreboardCoordinator)]
...
def _register_game_at_offset_websocket(hass: HomeAssistant) -> None:
    """Register the ``nba_live_scoreboard/game_at_offset`` WebSocket command.
    ...
    async def _handle_game_at_offset(hass, connection, msg) -> None:
        ...
        if coordinator is None:
            connection.send_error(msg["id"], "not_ready", "No NBA Live Scoreboard entry is configured")
            return
        try:
...
def _register_period_at_offset_websocket(hass: HomeAssistant) -> None:
    """Register the ``nba_live_scoreboard/period_at_offset`` WebSocket command.
...
def _register_player_card_websocket(hass: HomeAssistant) -> None:
    """Register the ``nba_live_scoreboard/player_card`` WebSocket command.
...
def _register_team_season_stats_websocket(hass: HomeAssistant) -> None:
    """Register the ``nba_live_scoreboard/team_season_stats`` WebSocket command.
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    coordinator = NbaLiveScoreboardCoordinator(hass, entry)
