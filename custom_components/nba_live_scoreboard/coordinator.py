"""ESPN NBA fetching, normalization, navigation, and Home Assistant events."""
...
_STAT_SCHEMAS: dict[str, tuple[str, list[str], list[str]]] = {
    "players": ("Players", ["MIN", "PTS", "FG", "3PT", "FT", "REB", "AST", "TO", "STL", "BLK", "OREB", "DREB", "PF", "+/-"],
...
class NbaLiveScoreboardData:
    ...
    featured_players: FeaturedPlayers = field(default_factory=dict)
...
class NbaLiveScoreboardCoordinator(DataUpdateCoordinator[NbaLiveScoreboardData]):
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        self.hass = hass
        self.entry = entry
        self.team_abbr = str(entry.data[CONF_TEAM]).upper()
        self.team_id = NBA_TEAM_MAP[self.team_abbr]
    ...
    def _all_periods(cls, summary: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for play in cls._all_plays(summary):
            ...
            grouped.setdefault(number, {"id": str(number), "number": number, "plays": [],
                                        "_is_current": False})["plays"].append(play)
        comp = _dict((_list(_dict(summary.get("header")).get("competitions")) or [{}])[0])
        number = _safe_int(_dict(comp.get("status")).get("period")) or max(grouped, default=0)
        if number and cls._resolve_status_info(comp)[1]:
            grouped.setdefault(number, {"id": str(number), "number": number, "plays": [],
                                        "_is_current": False})["_is_current"] = True
    ...
    def _normalize_period(cls, period: dict[str, Any], comp: dict[str, Any] | None = None) -> GamePeriod:
        ...
        label = cls._period_label(number)
        plays = [p for p in _list(period.get("plays")) if isinstance(p, dict)]
        scoring = [p for p in plays if p.get("scoringPlay") and _safe_int(p.get("scoreValue")) in {1, 2, 3}]
        away_id, home_id = cls._resolve_competitor_ids(comp)
        return {
            "id": str(period.get("id") or number), "number": number, "label": label,
            "description": f"Quarter {number}" if number <= 4 else label,
            "play_count": len(plays), "points": sum(_safe_int(p.get("scoreValue")) for p in scoring),
            "away_points": sum(_safe_int(p.get("scoreValue")) for p in scoring
                               if _team_id(p.get("team")) == away_id) if away_id else None,
            "home_points": sum(_safe_int(p.get("scoreValue")) for p in scoring
                               if _team_id(p.get("team")) == home_id) if home_id else None,
            "is_current": bool(period.get("_is_current")),
        }

    @staticmethod
    def _period_label(period: int) -> str:
        return f"Q{period}" if 0 < period <= 4 else ("OT" if period == 5 else f"{period - 4}OT" if period > 5 else "")
    ...
    def _normalize_period_context(cls, summary: dict[str, Any], comp: dict[str, Any] | None) -> PeriodContext:
        ...
        halftime = not _is_final(comp) and ("HALFTIME" in name or "halftime" in detail.lower())
        intermission = not _is_final(comp) and (halftime or "END_PERIOD" in name or "end of" in detail.lower())
        ...
        # Status and plays are countdown clocks, but an old play clock must
        # not be presented as the current running game clock.
        if not live or intermission:
            clock = ""
        label = cls._period_label(period)
        final_label = f"Final/{label}" if period > 4 else "Final"
        display = ("Halftime" if halftime else f"End {label}" if intermission else final_label if _is_final(comp)
                   else f"{clock} · {label}".strip(" ·") if live else "")
        return {"period": period, "display_period": label, "display_clock": clock,
                "period_prefix": "Halftime" if halftime else "End" if intermission else "",
                "label": display, "is_halftime": halftime, "is_intermission": intermission,
                "is_end_period": intermission and not halftime, "is_overtime": period > 4}
    ...
    def _normalize_situation(
        ...
        if comp is None:
            comp = _dict((_list(_dict(summary.get("header")).get("competitions")) or [{}])[0])
        result: Situation = {"possession_team_id": "", "away_timeouts": None, "home_timeouts": None, "shot_clock": None}
        if not cls._resolve_status_info(comp)[1]:
            return result
        source = {**_dict(comp.get("situation")), **_dict(summary.get("situation")), **_dict(live_situation)}
        possession = _team_id(source.get("possession")) or _team_id(source.get("possessionTeam"))
        if not possession:
            possessed = [_team_id(_dict(c).get("team")) for c in _list(comp.get("competitors"))
                         if _dict(c).get("possession") is True]
            possession = possessed[0] if len(possessed) == 1 else ""
        valid_teams = set(cls._resolve_competitor_ids(comp)) - {""}
        result["possession_team_id"] = possession if possession in valid_teams else ""
        for side in ("away", "home"):
            raw = source.get(f"{side}Timeouts")
            value = _dict(raw).get("timeoutsRemainingCurrent") if isinstance(raw, dict) else raw
            count = _optional_int(value)
            result[f"{side}_timeouts"] = count if count is not None and 0 <= count <= 7 else None
        raw_clock = source.get("shotClock")
        raw_clock = _dict(raw_clock).get("value", _dict(raw_clock).get("displayValue")) if isinstance(raw_clock, dict) else raw_clock
        try:
            shot_clock = float(raw_clock) if raw_clock is not None and not isinstance(raw_clock, bool) else None
        except (TypeError, ValueError):
            shot_clock = None
        result["shot_clock"] = shot_clock if shot_clock is not None and math.isfinite(shot_clock) and 0 <= shot_clock <= 24 else None
        return result
    ...
    def _normalize_recent_plays(
        ...
        comp = _dict((_list(_dict(summary.get("header")).get("competitions")) or [{}])[0])
        teams = set(cls._resolve_competitor_ids(comp)) - {""}
        for play in cls._all_plays(summary):
            ...
            low = type_text.lower()
            attempted = _optional_int(play.get("pointsAttempted"))
            score_value = _safe_int(play.get("scoreValue")) if play.get("scoringPlay") else 0
            is_shot = bool(play.get("shootingPlay")) and attempted in {1, 2, 3}
            free_throw = play.get("isFreeThrow") is True or attempted == 1 or "free throw" in low
            team_id = _team_id(play.get("team"))
            coordinate = _dict(play.get("coordinate"))
            valid_coordinate = all(isinstance(coordinate.get(key), (int, float))
                                   and not isinstance(coordinate.get(key), bool)
                                   and math.isfinite(coordinate[key]) for key in ("x", "y"))
            valid_coordinate = (valid_coordinate and 0 <= coordinate["x"] <= 50
                                and -4 <= coordinate["y"] <= 90 and team_id in teams
                                and is_shot and not free_throw)
            result.append({
                "id": str(play.get("id") or ""), "text": text,
                "away_score": _optional_int(play.get("awayScore")), "home_score": _optional_int(play.get("homeScore")),
                "wallclock_ts": _parse_iso_ts(play.get("wallclock")), "scoring_play": bool(play.get("scoringPlay")),
                "score_value": score_value if score_value in {1, 2, 3} else 0, "play_type": type_text,
                ...
                "team_id": team_id, "is_foul": "foul" in low,
                "is_turnover": "turnover" in low, "is_shot": is_shot, "is_free_throw": free_throw,
                # Verified against ESPN's ShotChartCourt in official client
                # 7469-808bf7bf.js (fitt-v3/ed5a90bc5707-2.0.5496): bottom is
                # x/50, left is (y+4)/94, both mirrored for the home team.
                # Preserve raw coordinates; the card applies that transform.
                # Never emit FT sentinels, unknown-team or invalid locations.
                "coordinate": {"x": coordinate["x"], "y": coordinate["y"]} if valid_coordinate else {},
                "shot_type": type_text if is_shot else "",
                "points_attempted": attempted if attempted in {1, 2, 3} else None,
            })
        return result
    ...
    def _normalize_scoring_plays(cls, summary: dict[str, Any]) -> list[ScoringPlay]:
        ...
        for play in plays:
            play = _dict(play)
            value = _safe_int(play.get("scoreValue"))
            if not play.get("text") or not play.get("scoringPlay") or value not in {1, 2, 3}:
                continue
            period = _safe_int(_dict(play.get("period")).get("number"))
            result.append({
                ...
                "clock": str(_dict(play.get("clock")).get("displayValue") or ""),
                "away_score": _optional_int(play.get("awayScore")), "home_score": _optional_int(play.get("homeScore")),
                "score_value": value, "team_id": _team_id(play.get("team")),
    ...
    def _records_from_standings(cls, payload: dict[str, Any] | None) -> dict[str, str]:
        ...
        for entry in cls._standings_entries(payload):
            ...
            # Prefer the compact native W-L record when provided.
    ...
    def _normalize_standings(
        ...
        for entry in cls._standings_entries(payload):
            ...
            entries.append({
                ...
                "team_short_name": str(team.get("shortDisplayName") or team.get("name") or team.get("abbreviation") or ""),
                "wins": stats.get("wins", ""), "losses": stats.get("losses", ""),
                "win_percent": stats.get("winPercent", ""), "games_behind": "", "streak": stats.get("streak", ""),
            })

        def rank(entry: dict[str, Any]) -> float:
            wins, losses = _optional_int(entry.get("wins")), _optional_int(entry.get("losses"))
            return wins / (wins + losses) if wins is not None and losses is not None and wins + losses > 0 else 0

        # ESPN's conference entries are not reliably sorted. Rank the
        # selected division by winning percentage, preserving source order
        # for ties rather than pretending to resolve unpublished tiebreakers.
        entries.sort(key=rank, reverse=True)
        if entries:
            leader_wins, leader_losses = _optional_int(entries[0]["wins"]), _optional_int(entries[0]["losses"])
            for entry in entries:
                wins, losses = _optional_int(entry["wins"]), _optional_int(entry["losses"])
                if all(value is not None and value >= 0 for value in (leader_wins, leader_losses, wins, losses)):
                    # Source gamesBehind is relative to the conference,
                    # not this division. Derive division GB from W/L.
                    behind = ((leader_wins - wins) + (losses - leader_losses)) / 2
                    entry["games_behind"] = "-" if behind == 0 else f"{behind:g}"
    ...
    def _roster_athletes(payload: dict[str, Any]) -> dict[str, dict[str, Any]]:
        ...
        for group in _list(payload.get("athletes")):
            group = _dict(group)
            # NBA roster responses may be flat or grouped by position.
    ...
    def _boxscore_player_row(entry: dict[str, Any], roster: dict[str, dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "did_not_play": entry.get("didNotPlay") if isinstance(entry.get("didNotPlay"), bool) else None,
        }
    ...
    def _normalize_team_stats(
        ...
        for side in ("away", "home"):
            ...
            for category in _list(block.get("statistics")):
                category = _dict(category)
                name = str(category.get("name") or category.get("type") or "players")
                ...
                columns = [columns[index] if index < len(columns) else key for index, key in enumerate(keys)]
            ...
            if not categories and roster:
                source = "roster"
                positions = {
                    "players": {"PG", "SG", "SF", "PF", "C", "G", "F", "G-F", "F-G", "F-C", "C-F"},
                }
    ...
    @staticmethod
    def _featured_player_stat_line(values: dict[str, str]) -> dict[str, str]:
        # Season averages retain their avg-prefixed machine keys; game
        # totals retain their unprefixed keys. Never divide or mix the two.
        return {
            "points": values.get("avgPoints", values.get("points", "")),
            "rebounds": values.get("avgRebounds", values.get("rebounds", values.get("totalRebounds", ""))),
            "assists": values.get("avgAssists", values.get("assists", "")),
            "steals": values.get("avgSteals", values.get("steals", "")),
            "blocks": values.get("avgBlocks", values.get("blocks", "")),
            "minutes": values.get("avgMinutes", values.get("minutes", "")),
            "field_goals": values.get("avgFieldGoalsMade-avgFieldGoalsAttempted", values.get("fieldGoalsMade-fieldGoalsAttempted", "")),
            "three_pointers": values.get("avgThreePointFieldGoalsMade-avgThreePointFieldGoalsAttempted",
                                         values.get("threePointFieldGoalsMade-threePointFieldGoalsAttempted", "")),
            "free_throws": values.get("avgFreeThrowsMade-avgFreeThrowsAttempted", values.get("freeThrowsMade-freeThrowsAttempted", "")),
        }

    @classmethod
    def _normalize_featured_players(
        cls, summary: dict[str, Any], team_stats: TeamStats, *, live: bool | None = None,
    ) -> FeaturedPlayers:
        result: FeaturedPlayers = {"away": {}, "home": {}}
        comp = _dict((_list(_dict(summary.get("header")).get("competitions")) or [{}])[0])
        if live is None:
            live = cls._resolve_status_info(comp)[1]
        game_observed = live or _is_final(comp) or (not comp and bool(cls._all_plays(summary)))
        if not game_observed:
            return result
        for side in ("away", "home"):
            team = _dict(team_stats.get(side))
            if team.get("source") != "game":
                continue
            category = next((_dict(c) for c in _list(team.get("categories"))
                             if _dict(c).get("name") == "players"), {})
            eligible = []
            for row in _list(category.get("rows")):
                row = _dict(row)
                if not row.get("id") or row.get("did_not_play") is True:
                    continue
                values = cls._stats_by_key(category, row)
                points = _optional_int(values.get("points"))
                minutes = str(values.get("minutes") or "")
                if points is not None and points >= 0 and (points > 0 or minutes not in {"", "-", "--", "0", "0:00", "0.0"}):
                    eligible.append(row)
            if not eligible:
                continue
            # The featured athlete is this game's scoring leader, not an
            # inferred starter or a claim about who is currently on court.
            chosen = max(eligible, key=lambda row: _safe_int(cls._stats_by_key(category, row).get("points")))
            result[side] = {
                "id": str(chosen.get("id") or ""), "display_name": str(chosen.get("name") or ""),
                "short_name": str(chosen.get("short_name") or chosen.get("name") or ""),
                "headshot": str(chosen.get("headshot") or ""), "team_id": str(team.get("team_id") or ""),
                "source": "game", "game_stats": cls._featured_player_stat_line(cls._stats_by_key(category, chosen)),
                "season_stats": {},
            }
        return result
    ...
    def _extract_season_line(cls, payload: dict[str, Any]) -> dict[str, Any]:
        # NBA career responses separate averages, totals and miscellaneous.
        # The Season view explicitly uses per-game averages, not totals
        # silently relabeled as PPG/RPG/APG.
        category = next((_dict(c) for c in _list(payload.get("categories"))
                         if _dict(c).get("name") == "averages"), {})
        rows = [r for r in _list(category.get("statistics")) if isinstance(r, dict)]
        season = max((_safe_int(_dict(row.get("season")).get("year")) for row in rows), default=0)
        rows = [r for r in rows if _safe_int(_dict(r.get("season")).get("year")) == season]
        if not rows or not season:
            return {}
        # The overall row of a traded player is authoritative for rates.
        ...
        return {"season": str(_dict(total.get("season")).get("displayName") or season),
                "categories": {"players": {
                    "columns": [_text(x) for x in _list(category.get("labels"))], "keys": names,
                    "descriptions": [_text(x) for x in _list(category.get("descriptions"))],
                    "stats": [_text(x) for x in _list(total.get("stats"))]}}}
    ...
    def _parse_player_card(
        ...
        primary = next((c for c in categories if c.get("name") == "averages"), {})
        career = {}
        if primary:
            teams = cls._team_abbr_map(stats_payload)
            career = {
                "kind": "averages", "label": "Regular-season per-game averages",
                "columns": [_text(value) for value in _list(primary.get("labels"))],
    ...
    async def _fetch_schedule(self) -> dict[str, Any]:
        ...
        try:
            ...
            # During the NBA offseason ESPN's generic `season` still names
            ...
            # ESPN publishes the NBA play-in as a separate season type.
            types = [value for value in (1, 2, 3, 5) if value != default_type]
            ...
            for phase, response in zip(types, responses, strict=True):
                if isinstance(response, dict) and isinstance(response.get("events"), list):
                    ...
                    raise UpdateFailed(f"Unable to fetch NBA season phase {phase}")
            ...
            if cached and now - cached[0] < SCHEDULE_TTL_SECONDS + SCHEDULE_STALE_FALLBACK_SECONDS:
                ...
                return cached[1]
            raise UpdateFailed(f"Unable to fetch NBA schedule: {err}") from err
    ...
    async def _get_summary(self, event_id: str, looks_live: bool) -> dict[str, Any]:
        ...
        try:
            ...
            if cached and now - cached[0] < SUMMARY_STALE_FALLBACK_SECONDS:
                return cached[1]
            raise UpdateFailed(f"Unable to fetch NBA game {event_id}: {err}") from err
    ...
    async def _assemble_game_data(
        ...
        featured_players = self._normalize_featured_players(summary, team_stats, live=is_live)
        featured_ids = [str(_dict(featured_players.get(side)).get("id") or "") for side in ("away", "home")]
        featured_seasons = await self._get_team_season_stats([aid for aid in featured_ids if aid])
        for side in ("away", "home"):
            player = _dict(featured_players.get(side))
            season = featured_seasons.get(str(player.get("id") or ""), {})
            category = _dict(_dict(season.get("categories")).get("players"))
            if player and season:
                player["season_stats"] = {**self._featured_player_stat_line(self._stats_by_key(category, category)),
    ...
    async def async_period_at_offset(self, offset: int) -> dict[str, Any] | None:
        ...
        label = self._period_label(number)
        period_context = {**context, "period": number, "display_period": label, "display_clock": "",
                          "label": label, "is_overtime": number > 4,
                          "is_intermission": False, "is_halftime": False, "is_end_period": False, "period_prefix": ""}
        comp = _dict((_list(_dict(summary.get("header")).get("competitions")) or [{}])[0])
    ...
    def _detect_game_events(
        prev: NbaLiveScoreboardData | None, curr: NbaLiveScoreboardData, team_id: int,
        ...
        base = {
            ...
            "possession_team_id": curr.situation.get("possession_team_id") or "",
            "shot_clock": curr.situation.get("shot_clock"),
        ...
        if (valid_previous_scores and valid_current_scores and not curr.is_delayed
            ...
            for name, delta, scoring_side in (
                ...
                candidates = curr.scoring_plays or [p for p in curr.recent_plays if p.get("scoring_play")]
                matching = next((p for p in reversed(candidates) if p.get("team_id") == scoring_team
                                 and _optional_int(p.get(f"{scoring_side}_score")) == new_score), {})
                old_candidates = [*prev.scoring_plays, *(p for p in prev.recent_plays if p.get("scoring_play"))]
                old_match = next((p for p in old_candidates if matching.get("id")
                                  and p.get("id") == matching["id"]), {})
                old_play_score = _optional_int(old_match.get(f"{scoring_side}_score"))
                previous_score = _optional_int(_competitor_for_side(prev.selected_competition, scoring_side).get("score"))
                if old_play_score is not None and previous_score is not None and old_play_score <= previous_score:
                    # Reviewing an already-counted two-pointer into a
                    # three-pointer changes the score but is not a new
                    # basket. A play published ahead of the score is not
                    # suppressed when the score subsequently catches up.
                    continue
                scoring_text = str(matching.get("text") or "")
                events.append((name, {**base, "score_delta": delta, "scoring_play_text": scoring_text}))
        # A delay before tipoff is not a game start.
    ...
    def _filter_score_rebounds(
        self, events: list[tuple[str, dict[str, Any]]], data: NbaLiveScoreboardData,
    ) -> list[tuple[str, dict[str, Any]]]:
        now = time.time()
    ...
    def _compute_update_interval(self, data: NbaLiveScoreboardData, events: list[dict[str, Any]]) -> timedelta:
        if data.is_live:
    ...
    async def _async_update_data(self) -> NbaLiveScoreboardData:
        schedule, events = await self._resolve_schedule()
        prev_id, next_id, live_id, display_id, _ = self._select_event(events)
