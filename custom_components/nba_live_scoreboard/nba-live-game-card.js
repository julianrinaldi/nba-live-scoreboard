...
if (!window.customCards.some((card) => card.type === CARD_TAG)) {
  window.customCards.push({
    type: CARD_TAG, name: "NBA Live Game Card",
    description: "NBA Live Scoreboard (" + CARD_VERSION + ")",
...
function findNbaEntity(hass) {
  const states = hass && hass.states || {};
  const ids = Object.keys(states);
  return ids.find((id) => id.startsWith("sensor.nba_live_scoreboard")) ||
    ids.find((id) => {
      const a = states[id].attributes || {};
      return id.startsWith("sensor.") && a.league === "NBA" && a.team_abbr !== undefined &&
...
const EDITOR_SCHEMA = [
  ...
  { type: "grid", schema: [
    ...
    "show_play_results", "show_period_summary", "show_possession", "show_court",
...
const EDITOR_LABELS = {
  ...
  show_matchup: "Featured scoring leaders", show_records: "Team records",
  show_linescore: "Quarter scoring", show_plays: "Quarter play-by-play",
  show_play_results: "Play result indicators", show_period_summary: "Period summary",
  show_possession: "Possession when available", show_court: "Basketball court",
  show_shot_chart: "Shot chart", show_situation: "Timeouts and shot clock",
...
const EDITOR_HELPERS = {
  ...
  show_within_hours: "For example, 24 hides the card until the next game is within 24 hours. Live games stay visible. Blank or 0 disables hiding, including post-game hiding. The card reappears automatically, even with refresh rate 0; editing previews stay visible.",
  show_after_hours: "With a positive pre-game hours window, keep the card visible this many hours after the last game's recorded finish. For example, 4 keeps it visible for four hours. Blank or 0 adds no post-game time; the next game's pre-game window can still keep it visible.",
  ...
  show_matchup: "Shows observed scoring leaders, not a projected starting lineup or players currently on court.",
  show_court: "A schematic court diagram, not a live ball location.",
  show_shot_chart: "Plots shots only when ESPN provides verified court coordinates. Missing locations are not invented.",
  show_highlights: "Shows an ESPN highlights link in the final-game panel only when ESPN has published one.",
};

class NbaLiveGameCardEditor extends HTMLElement {
  setConfig(config) {
    this._config = config || {};
    this._render();
  }
...
function normalizeShowWithinHours(value, name = "show_within_hours") {
  if (value == null || (typeof value === "string" && !value.trim())) return 0;
  if (!["number", "string"].includes(typeof value) ||
      !Number.isFinite(Number(value)) || Number(value) < 0) {
    throw new Error(name + " must be a non-negative number of hours.");
  }
  return Number(value);
}
...
function gameWindowVisibility(attrs, hours, now = Date.now(), afterHours = 0) {
  ...
  // Only server-confirmed finishes count. Opening a card, browsing history or
  // reloading the dashboard must never start a fresh post-game countdown.
  const end = parseGameStart(attrs.last_game_end);
  const postExpiry = end + afterHours * 3600000;
  const postVisible = afterHours > 0 && Number.isFinite(end) && end <= now && now < postExpiry;
  const postWake = postVisible && Number.isFinite(postExpiry) ? postExpiry : null;
  ...
  if (!Number.isFinite(start) || start < now - PENDING_GAME_GRACE_MS ||
    ...
    return { visible: postVisible, wakeAt: postWake };
  }
  const boundary = start - hours * 3600000;
  const preWake = now < boundary ? boundary : start + PENDING_GAME_GRACE_MS + 1;
  return { visible: postVisible || now >= boundary,
    wakeAt: postWake == null ? preWake : Math.min(preWake, postWake) };
}
...
window.__nbaLiveLogoCache = window.__nbaLiveLogoCache || new Map();
...
function _prefetchImage(card, normalized) {
  const cache = window.__nbaLiveLogoCache;
...
function espnPlayerUrl(id) {
  return `https://www.espn.com/nba/player/_/id/${encodeURIComponent(String(id == null ? "" : id))}`;
}
...
function playerCardBodyHtml(card, headshotSrc) {
  ...
  const shot = headshotSrc
    ? `<img class="nba-pc-shot" src="${escapeHtml(headshotSrc)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer">`
    : `<div class="nba-pc-shot nba-pc-shot--ph" aria-hidden="true"></div>`;
  ...
  const sub = [
    ...
    bio.debut_year ? `NBA debut ${bio.debut_year}` : "",
  ].filter((x) => x && String(x).trim());

  const header =
    `<div class="nba-pc-bio">${shot}<div class="nba-pc-bio-meta">` +
    (chips.length
      ? `<div class="nba-pc-bio-line">${chips.map((c) => escapeHtml(c)).join(" · ")}</div>`
      : "") +
    (sub.length
      ? `<div class="nba-pc-bio-sub">${sub.map((c) => escapeHtml(c)).join(" · ")}</div>`
      : "") +
    `</div></div>`;
  ...
  if (columns.length && seasons.length) {
    ...
    const bodyRows = seasons
      .map((s) => {
        ...
        return (
          ...
          `<td class="nba-pc-team">${escapeHtml(s.team || "")}</td>${cells}</tr>`
        );
      })
      .join("");
    const totalRow = totals.length
      ? `<tr class="nba-pc-total"><th scope="row">Career</th><td class="nba-pc-team"></td>` +
        columns
          .map((_, i) => `<td>${escapeHtml(totals[i] ?? "")}</td>`)
          .join("") +
        `</tr>`
      : "";
    const kind = String(career.label || career.kind || "Basketball").replace(/_/g, " ");
    ...
    table =
      `<div class="nba-pc-table-wrap" tabindex="0" role="group" ` +
      `aria-label="${escapeHtml(kind)} career stats, scrollable"><table class="nba-pc-table">` +
      `<caption class="nba-pc-caption">${escapeHtml(kind)} — career by season</caption>` +
      `<thead><tr><th scope="col">Year</th><th scope="col">Team</th>${headCells}</tr></thead>` +
      `<tbody>${bodyRows}${totalRow}</tbody></table></div>`;
  } else {
    table = `<div class="nba-pc-msg nba-pc-nostats">No career stats available for this player.</div>`;
  }
...
function periodLabel(period) {
  ...
  return n > 4 ? (n === 5 ? "OT" : (n - 4) + "OT") : n > 0 ? "Q" + n : "";
}
function periodContext(attrs) {
  ...
  return {
    ...
    display_period: ctx.display_period || periodLabel(period),
  };
}
...
function renderSituationRow(situation, awayLabel = "Away", homeLabel = "Home") {
  const s = situation || {};
  const away = numberOrNull(s.away_timeouts);
  const home = numberOrNull(s.home_timeouts);
  const shotClock = numberOrNull(s.shot_clock);
  return '<div class="count-dots-row prominent verbose nba-situation">' +
    '<span class="count-pack"><span class="dots-label verbose">' + escapeHtml(awayLabel) + ' TO:</span><span>' + escapeHtml(away ?? "—") + '</span></span>' +
    '<span class="count-pack"><span class="dots-label verbose">Shot clock:</span><span>' + escapeHtml(shotClock ?? "—") + '</span></span>' +
    '<span class="count-pack"><span class="dots-label verbose">' + escapeHtml(homeLabel) + ' TO:</span><span>' + escapeHtml(home ?? "—") + '</span></span></div>';
}
function possessionSide(attrs) {
  const id = String(attrs.situation && attrs.situation.possession_team_id || "");
...
function renderPossessionRow(attrs) {
  const owner = possessionSide(attrs);
  const side = (name) => '<div class="base-slot"><span class="base-label">' + escapeHtml(teamAbbreviation(attrs, name)) + '</span>' +
    (owner === name ? '<span class="nba-possession-mark" aria-label="Possession" title="Possession">●</span>' : "") + '</div>';
  return '<div class="bases-occupancy-row nba-possession">' + side("away") +
    '<div class="nba-possession-label">Possession' + (owner ? "" : " —") + '</div>' + side("home") + '</div>';
}
...
function renderCourt(attrs, plays, detailed) {
  // A schematic court, never a claim about the basketball's live position.
  // Verified 2026-09-03 against ESPN's own ShotChartCourt and game 401809234:
  // https://cdn1.espn.net/fitt-v3/ed5a90bc5707-2.0.5496/client/espnfitt/7469-808bf7bf.js
  // ESPN uses bottom=x/50 and left=(y+4)/94, mirroring both for home shots.
  // Here the court occupies SVG x=3..97, y=3..53. No period-based flip is added.
  const teamId = (side) => String(attrs[side + "_team"]?.id ||
    (attrs.competition?.competitors || []).find((c) => c.homeAway === side)?.team?.id || "");
  const homeId = teamId("home"), awayId = teamId("away");
  const shots = detailed ? (Array.isArray(plays) ? plays : []).flatMap((play) => {
    if (!play?.is_shot || play.is_free_throw === true || Number(play.points_attempted) === 1 ||
      /free throw/i.test(String(play.play_type || play.shot_type || ""))) return [];
    const point = play.coordinate || {};
    const {x, y} = point;
    if (typeof x !== "number" || typeof y !== "number" || !Number.isFinite(x) || !Number.isFinite(y) ||
      x < 0 || x > 50 || y < -4 || y > 90) return [];
    const id = String(play.team_id || "");
    const side = homeId && id === homeId ? "home" : awayId && id === awayId ? "away" : "";
    // Without a team, the home/away orientation cannot be determined safely.
    if (!side) return [];
    return [{play, side, x: side === "home" ? 93 - y : y + 7, y: side === "home" ? x + 3 : 53 - x}];
  }) : [];
  const points = shots.map(({play, side, x, y}) => '<g><title>' + escapeHtml(play.text || play.shot_type || "Shot") + '</title>' +
    '<circle cx="' + x.toFixed(2) + '" cy="' + y.toFixed(2) + '" r="1.15" class="nba-court-play ' + side +
    (play.scoring_play ? ' made' : '') + '"/></g>').join("");
  const caption = shots.length ? shots.length + " shot locations · Filled: made · Open: missed" : "Shot locations unavailable";
  const aria = detailed ? "Selected-quarter shot chart. " + caption : "Basketball court diagram";
  return '<div class="' + (detailed ? "nba-shot-chart" : "matchup-center nba-court-center") + '" role="img" aria-label="' + escapeHtml(aria) + '">' +
    '<svg viewBox="0 0 100 56" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">' +
    '<rect x="3" y="3" width="94" height="50" class="nba-court-outline"/>' +
    '<path d="M50 3V53" class="nba-court-line"/><circle cx="50" cy="28" r="6" class="nba-court-line"/>' +
    '<path d="M3 20H22V36H3M97 20H78V36H97" class="nba-court-paint"/>' +
    '<circle cx="22" cy="28" r="6" class="nba-court-line"/><circle cx="78" cy="28" r="6" class="nba-court-line"/>' +
    '<path d="M3 6H17.2A23.75 23.75 0 0 1 17.2 50H3M97 6H82.8A23.75 23.75 0 0 0 82.8 50H97" class="nba-court-line"/>' +
    '<path d="M7 25V31M93 25V31" class="nba-court-hoop"/>' +
    '<circle cx="8.25" cy="28" r=".75" class="nba-court-hoop"/><circle cx="91.75" cy="28" r=".75" class="nba-court-hoop"/>' +
    points + '</svg>' + (detailed ? '<div class="nba-chart-caption">' + escapeHtml(caption) +
      (shots.length ? ' · <span class="nba-shot-away">' + escapeHtml(teamAbbreviation(attrs,"away")) + '</span> / <span class="nba-shot-home">' + escapeHtml(teamAbbreviation(attrs,"home")) + '</span>' : "") + '</div>' : "") + '</div>';
}
function featuredPlayerLines(player, pregame) {
  const p = player || {};
  const game = p.game_stats || {};
  const hasGame = Object.values(game).some((v) => v !== "" && v !== null && v !== undefined);
  const seasonView = pregame || !hasGame;
  const s = seasonView ? p.season_stats || {} : game;
  const stat = (value, label) => value !== null && value !== undefined && value !== "" ? value + " " + label : "";
  const primary = [stat(s.points,"PTS"), stat(s.rebounds,"REB"), stat(s.assists,"AST")].filter(Boolean).join(" • ");
  const secondary = [stat(s.field_goals,"FG"), stat(s.three_pointers,"3PT")].filter(Boolean).join(" • ");
  const context = seasonView && s.season ? s.season + " season · per game" : hasGame ? "Game scoring leader" : "";
  return {primary, secondary, context};
}
function featuredStatTokens(line) {
  // Keep each value and label together on narrow cards. Attach a separator
  // to its following token so a bullet cannot occupy a line by itself.
  return String(line || "").split(" • ").filter(Boolean).map((token, index) =>
    '<span class="nba-stat-token">' +
    (index ? '<span class="nba-stat-separator" aria-hidden="true">• </span>' : "") +
    escapeHtml(token) + '</span>').join(" ");
}
...
function renderFeaturedPlayerSide(card, attrs, side, pregame) {
  const q = attrs.featured_players && attrs.featured_players[side] || {};
  const meta = attrs[side + "_team"] || {};
  const name = q.short_name || q.display_name || "";
  const lines = featuredPlayerLines(q, pregame);
  const src = q.headshot || meta.logo || "";
  const portrait = pregame
    ? (src ? '<img class="upcoming-featured-player-img' + (!q.headshot ? ' logo-fallback' : '') + '" src="' + escapeHtml(requestCachedLogo(card, src)) + '" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer">' : '<div class="upcoming-featured-player-img placeholder"></div>')
    : renderPlayerHeadshot(card, src, name || teamAbbreviation(attrs, side));
  return '<div class="matchup-side with-headshot stacked centered-half' + (pregame ? ' upcoming-featured-player-side' : '') + '"' +
    teamPopupAttrs(card, attrs, side) + '>' + portrait +
    '<div class="matchup-copy centered-copy"><div class="' + (pregame ? "upcoming-featured-player-name" : "matchup-value") + '">' +
    playerNameMarkup(name ? shortPersonName(name) : "No player data", q.id) + '</div>' +
    '<div class="matchup-subtle strongish stat-line">' + featuredStatTokens(lines.primary || "—") + '</div>' +
    '<div class="matchup-subtle secondary stat-line">' + featuredStatTokens(lines.secondary) + '</div>' +
    (lines.context ? '<div class="nba-featured-player-context">' + escapeHtml(lines.context) + '</div>' : "") +
    '</div></div>';
}
function renderFeaturedMatchup(card, attrs, pregame) {
  const center = pregame ? '<div class="upcoming-featured_players-vs">vs</div>' :
    card.config.show_court !== false ? renderCourt(attrs, [], false) : "";
  return '<div class="' + (pregame ? "upcoming-featured_players-grid" : "matchup-block") + '">' +
    (pregame ? "" : '<div class="matchup-grid enhanced productionish nba-matchup' + (!center ? ' nba-no-court' : '') + '">') +
    renderFeaturedPlayerSide(card, attrs, "away", pregame) + center + renderFeaturedPlayerSide(card, attrs, "home", pregame) +
    (pregame ? "" : '</div>') + '</div>';
}
function renderBreakLeaders(card, attrs, ownerSide) {
  const list = (attrs.leaders && attrs.leaders[ownerSide] || []).filter(Boolean).slice(0, 3);
  if (!list.length) return "";
  const ctx = periodContext(attrs);
  const label = ctx.is_halftime ? "Halftime" : "End of " + periodLabel(ctx.period);
  return '<div class="break-leaders-panel"><div class="break-leaders-title">' + escapeHtml(label) + '&nbsp;&nbsp;Game Leaders</div><div class="break-leaders-grid">' +
    list.map((p) => '<div class="break-leaders-card">' + renderPlayerHeadshot(card, p.headshot || "", p.name || "") +
      ...
      '<div class="nba-leader-category">' + escapeHtml(p.category || "") + '</div>' +
...
function renderPeriodSummary(period) {
  ...
  const parts = [
    ...
    d.points !== null && d.points !== undefined && d.points !== "" ? d.points + (Number(d.points) === 1 ? " point" : " points") : "",
  ].filter(Boolean);
  const label = parts.join(" • ") || d.description || "";
  return label ? '<div class="on-deck-row nba-period-summary"><span class="on-deck-label">Period:</span><span>' + escapeHtml(label) + '</span></div>' : "";
}
function playIndicator(play) {
  if (play.scoring_play || Number(play.score_value) > 0) {
    ...
    return away !== null && home !== null ? away + "-" + home : Number(play.score_value) > 0 ? "+" + Number(play.score_value) : "PTS";
  }
  if (play.is_foul) return "FOUL";
  if (play.is_turnover) return "TO";
  if (play.is_shot) return play.abbreviation || "MISS";
...
function renderRecentPlays(plays, config) {
  ...
  return '<div class="plays-panel" tabindex="0" role="region" aria-label="Period play-by-play, newest first">' +
    '<div class="nba-play-order">Newest first</div>' + list.map((p) => {
    const clock = [periodLabel(p.period_number || p.period), p.clock || p.display_clock].filter(Boolean).join(" ");
    return '<div class="play-row"><div class="play-text">' +
      (clock ? '<span class="nba-play-clock">' + escapeHtml(clock) + '</span> ' : "") +
...
function renderScoringPlaysPanel(attrs, awayMeta, homeMeta) {
  const list = (Array.isArray(attrs.scoring_plays) ? attrs.scoring_plays : []).filter(Boolean);
  if (!list.length) return "";
  let prevAway = 0, prevHome = 0;
  return '<div class="scoring-plays-panel"><div class="panel-heading">Scoring Plays</div><div class="nba-scoring-list" tabindex="0" role="region" aria-label="Scoring plays, newest first"><div class="nba-play-order">Newest first</div>' + list.map((p) => {
    ...
    return '<div class="scoring-play-row"><span class="scoring-play-period">' + escapeHtml(period) + '</span>' +
      ...
      (p.clock ? '<span class="nba-play-clock">' + escapeHtml(p.clock) + '</span> ' : "") + escapeHtml(p.text || "") +
...
function renderUpcomingDetails(card, attrs, awayMeta, homeMeta, awayLogo, homeLogo, options) {
  ...
  const standingsHtml = entries.length && showStandings ? '<div class="upcoming-standings">' +
    ...
    '<div class="standings-row standings-header"><span>Team</span><span class="standings-wl">W-L</span><span class="standings-gb">GB</span></div>' +
    entries.map((e) => '<div class="standings-row' + (String(e.team_id || "") === String(attrs.team_id || "") ? ' my-team' : '') + '">' +
      ...
      '<span class="standings-wl">' + escapeHtml([e.wins ?? "—", e.losses ?? "—"].join("-")) + '</span>' +
      '<span class="standings-gb">' + escapeHtml(e.games_behind ?? "—") + '</span></div>').join("") + '</div>' : "";
  ...
  return '<div class="upcoming-details-panel">' +
    (!final && card.config.show_matchup !== false ? renderFeaturedMatchup(card, attrs, true) : "") +
...
function teamStatsTablesHtml(team, view, seasonStats) {
  ...
  if (!categories.some((c) => (c.rows || []).length)) return '<div class="nba-lu-msg">Team statistics are not available yet.</div>';
  const season = view === "season";
  return (team.source === "roster" && !season ? '<div class="nba-lu-msg">Roster — game statistics are not available yet.</div>' : "") + categories.map((category) => {
    ...
    const seasonLabel = (years.length === 1 ? " · " + years[0] + " season" : mixedSeasons ? " · Latest available seasons" : "") +
      (season ? " · per game" : "");
    const header = '<tr><th scope="col" class="nba-lu-name">Player</th>' +
    ...
    const body = rows.map((r) => {
      ...
      return '<tr><th scope="row" class="nba-lu-name">' + escapeHtml(shortPersonName(r.name || r.short_name || "—")) +
        (!season && r.did_not_play === true ? ' <span class="nba-dnp">DNP</span>' : "") + '</th>' +
    ...
    const totals = !season && Array.isArray(category.totals) && category.totals.length ?
      '<tr class="nba-lu-total"><th scope="row" class="nba-lu-name">Team</th>' + columns.map((_, i) => '<td>' + escapeHtml(category.totals[i] ?? "—") + '</td>').join("") + '</tr>' : "";
    return '<div class="nba-lu-table-wrap" tabindex="0" role="group" aria-label="' + escapeHtml(category.label || category.name) +
      ', scrollable"><table class="nba-lu-table"><caption class="nba-lu-caption">' + escapeHtml(category.label || category.name) +
...
class NbaLiveGameCard extends HTMLElement {
  ...
  static getStubConfig(hass) {
    return { entity: findNbaEntity(hass) };
  }
  ...
  setConfig(config) {
    ...
    nextConfig.show_after_hours = normalizeShowWithinHours(nextConfig.show_after_hours, "show_after_hours");
  ...
  _updateGameVisibility(stateObj) {
    ...
    const result = !enabled || diagnostic || this.preview || this.editMode
      ...
      : gameWindowVisibility(stateObj.attributes || {}, this.config.show_within_hours,
        Date.now(), this.config.show_after_hours);
  ...
  _upcomingDetailsFingerprint(attrs) {
    return JSON.stringify([attrs.featured_players, attrs.division_standings,
  ...
  _fetchPlayerCard(athleteId) {
    ...
    const req = this._hass.connection
      .sendMessagePromise({
        type: "nba_live_scoreboard/player_card",
        athlete_id: id,
      })
      .then((res) => {
  ...
  _ensurePlayerCardPopup() {
    ...
    if (!document.getElementById("nba-pc-style")) {
      const style = document.createElement("style");
      style.id = "nba-pc-style";
      style.textContent = `
        .nba-pc-overlay {
        ...
        .nba-pc-overlay[hidden] { display: none; }
        .nba-pc-dialog {
        ...
        .nba-pc-close:hover { color: var(--primary-text-color, #fff); }
        .nba-pc-close:focus-visible,
        .nba-pc-espn:focus-visible,
        .nba-pc-table-wrap:focus-visible {
        ...
        .nba-pc-msg { color: var(--secondary-text-color, #9b9b9b); }
        .nba-pc-retry {
        ...
        .nba-pc-spinner {
          ...
          animation: nba-pc-spin 0.8s linear infinite; margin: 0 auto 10px;
        }
        @keyframes nba-pc-spin { to { transform: rotate(360deg); } }
        @media (prefers-reduced-motion: reduce) {
          .nba-pc-spinner { animation-duration: 2s; }
        }
        .nba-pc-footer {
        ...
        .nba-pc-espn:hover { text-decoration: underline; }
        .nba-pc-body--ready {
        ...
        .nba-pc-shot--ph { display: block; }
        .nba-pc-bio-meta { min-width: 0; }
        .nba-pc-bio-line {
        ...
        .nba-pc-table th, .nba-pc-table td {
          padding: 5px 8px; text-align: right;
          border-bottom: 1px solid var(--divider-color, rgba(255,255,255,0.08));
        }
        .nba-pc-table thead th {
          color: var(--secondary-text-color, #9b9b9b);
          font-weight: 600; cursor: help;
        }
        .nba-pc-table th[scope="row"] { text-align: left; }
        .nba-pc-table td.nba-pc-team { text-align: left; }
        .nba-pc-total th, .nba-pc-total td {
          font-weight: 700;
          color: var(--primary-text-color, #e1e1e1);
    ...
    overlay.className = "nba-pc-overlay";
    overlay.hidden = true;


    const titleId = `nba-pc-title-${Math.random().toString(36).slice(2, 9)}`;
    overlay.innerHTML = `
      <div class="nba-pc-dialog" role="dialog" aria-modal="true"
           aria-labelledby="${titleId}" tabindex="-1">
        <div class="nba-pc-header">
          <div class="nba-pc-title" id="${titleId}" role="heading" aria-level="2">Player</div>
          <button class="nba-pc-close" type="button" aria-label="Close">✕</button>
        </div>
        <div class="nba-pc-body" aria-live="polite"></div>
        <div class="nba-pc-footer">
          <a class="nba-pc-espn" target="_blank" rel="noopener noreferrer">View on ESPN ↗</a>
        </div>
      </div>`;

    this._pcOverlay = overlay;
    this._pcDialog = overlay.querySelector(".nba-pc-dialog");
    this._pcTitle = overlay.querySelector(".nba-pc-title");
    this._pcBody = overlay.querySelector(".nba-pc-body");
    this._pcEspn = overlay.querySelector(".nba-pc-espn");

    overlay.addEventListener("click", (ev) => {
      if (ev.target === overlay) return this._closePlayerCardPopup();
      const t = ev.target instanceof Element ? ev.target : null;
      if (t && t.closest(".nba-pc-close")) return this._closePlayerCardPopup();
      if (t && t.closest(".nba-pc-retry") && this._pcAthleteId) {
  ...
  _setPlayerCardState(state, card) {
    ...
    this._pcBody.className =
      state === "ready" ? "nba-pc-body nba-pc-body--ready" : "nba-pc-body";
    if (state === "loading") {
      this._pcBody.innerHTML = `<div><div class="nba-pc-spinner"></div><div class="nba-pc-msg">Loading player…</div></div>`;
    } else if (state === "error") {
      this._pcBody.innerHTML =
        `<div><div class="nba-pc-msg">Couldn't load this player.</div>` +
        `<button class="nba-pc-retry" type="button" data-pc-retry>Retry</button></div>`;
    } else if (state === "empty") {
      this._pcBody.innerHTML = `<div class="nba-pc-msg">No stats available for this player.</div>`;
    } else {
  ...
  _fetchTeamSeasonStats(side) {
    ...
    const req = this._hass.connection.sendMessagePromise({
      type: "nba_live_scoreboard/team_season_stats", athlete_ids: ids,
    }).then((res) => {
  ...
  _ensureLineupPopup() {
    ...
    if (!document.getElementById("nba-lu-style")) {
      const style = document.createElement("style");
      style.id = "nba-lu-style";
      style.textContent = `
        .nba-lu-overlay {
          position: fixed; inset: 0; z-index: 99999;
          display: flex; align-items: center; justify-content: center;
        ...
        .nba-lu-overlay[hidden] { display: none; }
        .nba-lu-dialog {
        ...
        .nba-lu-views input { position: absolute; opacity: 0; pointer-events: none; }
        .nba-lu-views label:has(input:checked) {
          background: var(--warning-color, #ffb300);
          color: var(--text-primary-color, #111);
        }
        .nba-lu-views label:has(input:focus-visible) {
          outline: 2px solid var(--warning-color, #ffb300); outline-offset: 2px;
        }
        .nba-lu-close {
          flex: none; cursor: pointer; border: 0; border-radius: 50%;
        ...
        .nba-lu-close:hover { color: var(--primary-text-color, #fff); }
        .nba-lu-close:focus-visible,
        .nba-lu-table-wrap:focus-visible {
          outline: 2px solid var(--warning-color, #ffb300); outline-offset: 2px;
        }
        .nba-lu-body {
        ...
        .nba-lu-msg { color: var(--secondary-text-color, #9b9b9b); }
        .nba-lu-retry {
          margin-top: 10px; cursor: pointer;
          background: transparent; color: var(--warning-color, #ffb300);
          border: 1px solid var(--divider-color, rgba(255,255,255,0.2));
        ...
        .nba-lu-spinner {
          ...
          border: 3px solid var(--divider-color, rgba(255,255,255,0.2));
          border-top-color: var(--warning-color, #ffb300);
          animation: nba-lu-spin 0.8s linear infinite; margin: 0 auto 10px;
        }
        @keyframes nba-lu-spin { to { transform: rotate(360deg); } }
        @media (prefers-reduced-motion: reduce) {
          .nba-lu-spinner { animation-duration: 2s; }
        }
        .nba-lu-table-wrap {
          overflow-x: auto; -webkit-overflow-scrolling: touch;
          margin-bottom: 14px;
        }
        .nba-lu-table-wrap:last-child { margin-bottom: 0; }
        .nba-lu-table {
          border-collapse: collapse; width: 100%;
          font-size: 0.82em; white-space: nowrap;
        }
        ...
        .nba-lu-table th, .nba-lu-table td {
          padding: 5px 8px; text-align: right;
          border-bottom: 1px solid var(--divider-color, rgba(255,255,255,0.08));
        }
        .nba-lu-table thead th {
          color: var(--secondary-text-color, #9b9b9b); font-weight: 600;
        }
        .nba-lu-table th.nba-lu-name, .nba-lu-table td.nba-lu-name,
        .nba-lu-table th.nba-lu-pos, .nba-lu-table td.nba-lu-pos,
        .nba-lu-table th.nba-lu-num, .nba-lu-table td.nba-lu-num {
          text-align: left;
        }
        .nba-lu-table td.nba-lu-name { color: var(--primary-text-color, #e1e1e1); }
        ...
        .nba-lu-table .nba-lu-num { width: 1.5em; }
        .nba-lu-table tr.nba-lu-out td, .nba-lu-table tr.nba-lu-out th {
          opacity: 0.55;
        }
      `;
      document.head.appendChild(style);
    ...
    const overlay = document.createElement("div");
    overlay.className = "nba-lu-overlay";
    overlay.hidden = true;


    const uid = Math.random().toString(36).slice(2, 9);
    const titleId = `nba-lu-title-${uid}`;
    const grp = `nba-lu-view-${uid}`;
    overlay.innerHTML = `
      <div class="nba-lu-dialog" role="dialog" aria-modal="true"
           aria-labelledby="${titleId}" tabindex="-1">
        <div class="nba-lu-header">
          <img class="nba-lu-logo" alt="" aria-hidden="true" hidden
               loading="lazy" decoding="async" referrerpolicy="no-referrer">
          <div class="nba-lu-title" id="${titleId}" role="heading" aria-level="2">Team stats</div>
          <div class="nba-lu-views" role="radiogroup" aria-label="Stats view">
            <label><input type="radio" name="${grp}" class="nba-lu-view" value="game" checked>Game</label>
            <label><input type="radio" name="${grp}" class="nba-lu-view" value="season">Season</label>
          </div>
          <button class="nba-lu-close" type="button" aria-label="Close">✕</button>
        </div>
        <div class="nba-lu-body" aria-live="polite"></div>
      </div>`;

    this._luOverlay = overlay;
    this._luDialog = overlay.querySelector(".nba-lu-dialog");
    this._luTitle = overlay.querySelector(".nba-lu-title");
    this._luLogo = overlay.querySelector(".nba-lu-logo");
    this._luBody = overlay.querySelector(".nba-lu-body");

    overlay.addEventListener("click", (ev) => {
      if (ev.target === overlay) return this._closeLineupPopup();
      const t = ev.target instanceof Element ? ev.target : null;
      if (t && t.closest(".nba-lu-close")) return this._closeLineupPopup();
      if (t && t.closest(".nba-lu-retry")) return this._renderLineupBody();
    });
    overlay.addEventListener("change", (ev) => {
      const t = ev.target instanceof Element ? ev.target : null;
      if (t && t.classList.contains("nba-lu-view")) {
  ...
  _setLineupState(state) {
    ...
    this._luBody.className =
      state === "ready" ? "nba-lu-body" : "nba-lu-body nba-lu-body--msg";
    if (state === "loading") {
      this._luBody.innerHTML = `<div><div class="nba-lu-spinner"></div><div class="nba-lu-msg">Loading season stats…</div></div>`;
    } else if (state === "error") {
      this._luBody.innerHTML =
        `<div><div class="nba-lu-msg">Couldn't load season stats.</div>` +
        `<button class="nba-lu-retry" type="button">Retry</button></div>`;
    } else if (state === "pregame") {
      this._luBody.innerHTML = `<div class="nba-lu-msg">Team statistics are not available yet.</div>`;
    }
  }
  ...
  _setTeamBodyHtml(html) {
    ...
    const oldTables = [...this._luBody.querySelectorAll(".nba-lu-table-wrap")];
    ...
    const tables = [...this._luBody.querySelectorAll(".nba-lu-table-wrap")];
  ...
  _openLineupPopup(side, trigger = null) {
    ...
    const radios = this._luOverlay.querySelectorAll(".nba-lu-view");
    radios.forEach((r) => {
  ...
  _onContentClick(ev) {
    ...
    if (ev.target instanceof Element && ev.target.closest("a[href], .nba-scoring-list")) return;
  ...
  _onContentKeydown(ev) {
    ...
    if (ev.target instanceof Element && ev.target.closest("a[href], button, input, select, textarea, .nba-scoring-list")) return;
  ...
  _navigateSchedule(delta) {
    ...
    this._navInflight = conn
      .sendMessagePromise({
        type: "nba_live_scoreboard/game_at_offset",
        entity_id: entity,
        offset: target,
      })
      .then((res) => {
  ...
  _navigatePeriod(delta) {
    ...
    this._periodInflight = conn
      .sendMessagePromise({
        type: "nba_live_scoreboard/period_at_offset",
        entity_id: entity,
        offset: target,
      })
      .then((res) => {
  ...
  set hass(hass) {
    ...
    if (!this.card) {
      ...
      this.card.className = "nba-live-game-card";
  ...
  renderLinescore(competition, history, attrs = {}) {
    ...
    const periods = Math.max(4, Math.min(20, historyPeriod || Math.max(a.length, h.length)));
    ...
    return '<div class="linescore"><div class="linescore-grid" style="grid-template-columns:max-content repeat(' + periods + ',minmax(18px,1fr)) max-content">' +
      '<div></div>' + header + '<div class="period-head">PTS</div>' + row("away", away, a) + row("home", home, h) + '</div></div>';
  }

  _computeRenderFingerprint(stateObj) {
    // Include clocks, possession, timeouts, featured players and corrected
    // play text: visible basketball data can change without a score.
  ...
  render() {
    ...
    if (!competitors.length) {
      this.content.innerHTML = '<div class="empty">' +
        (stateObj.state === "unavailable" ? "NBA game data is temporarily unavailable." :
          "No NBA game is currently available for " + escapeHtml(attrs.team_name || attrs.team_abbr || "this team") + ".") + '</div>';
      return;
    }
    const away = competitors.find((c) => c.homeAway === "away") || {};
    const home = competitors.find((c) => c.homeAway === "home") || {};
    ...
    const possession = this.config.show_possession !== false ? possessionSide(contextAttrs) : "";
    ...
    const scoreboard = '<div class="scoreboard-main"><div class="scoreboard scoreboard-rich">' +
      this.teamRow(awayTeam,awayMeta,"",awayScore,false,false,possession === "away") +
      this.teamRow(homeTeam,homeMeta,"",homeScore,false,true,possession === "home") +
    ...
    let extras = "";
    if (live && !collapsed) {
      const breakLeaders = !history && this.config.show_matchup !== false && (ctx.is_halftime || ctx.is_intermission || ctx.is_end_period) ?
        renderBreakLeaders(this,attrs,ownerSide) : "";
      const matchup = !history && !breakLeaders && this.config.show_matchup !== false ? renderFeaturedMatchup(this,attrs,false) :
        !history && !breakLeaders && this.config.show_court !== false ? '<div class="nba-court-only">' + renderCourt(attrs,[],false) + '</div>' : "";
      ...
      const historyLabel = history ? '<div class="nba-history-label">Earlier period · ' + escapeHtml([periodLabel(ctx.period),ctx.display_clock].filter(Boolean).join(" ")) + '</div>' : "";
      extras = '<div class="live-panel productionish">' + historyLabel +
        ...
        (!history && this.config.show_possession !== false ? renderPossessionRow(attrs) : "") +
        (this.config.show_shot_chart === true ? renderCourt(contextAttrs,contextAttrs.recent_plays || [],true) : "") +
        plays + '</div>';
    }
  ...
  renderCompactNonLive(
    ...
    const finalSuffix = isFinal && finalContext.period > 4 ? periodLabel(finalContext.period) : "";
  ...
  renderPeriodMarker(stateInfo, context, history) {
    ...
    const label = context.is_halftime && !history ? "HT" : periodLabel(context.period) || "LIVE";
    return '<div class="period-stack nba-period-stack"><div class="num">' + escapeHtml(label) + '</div>' +
      (context.display_clock && !context.is_halftime && !context.is_intermission && !context.is_end_period ? '<div class="nba-clock">' + escapeHtml(context.display_clock) + '</div>' : "") + '</div>';
  }

  teamRow(team, teamMeta, record, score, winner, isHome, hasPossession) {
    ...
    return '<div class="team-row ' + (winner ? "winner " : "") + (isHome ? "home" : "away") + '"><div class="team-left">' +
      ...
      '</div></div></div><div class="team-right nba-score-values">' +
      (hasPossession ? '<span class="nba-possession-mark" aria-label="Possession" title="Possession">●</span>' : '<span class="nba-possession-space"></span>') +
      '<div class="score rhe-score">' + escapeHtml(score.text || "—") + '</div></div></div>';
  }
}


const CARD_STYLE_ID = "nba-live-game-card-style";
const CARD_CSS = `nba-live-game-card .wrapper {
          padding: 0 1px 0;

          container-type: inline-size;
          container-name: nba-card;
        }nba-live-game-card .header {
          display: flex;
          align-items: center;
          justify-content: space-between;
          gap: 6px;
          margin-bottom: 6px;
        }nba-live-game-card .title-wrap {
          display:flex;
          align-items:center;
          gap:6px;
          min-width:0;
        }nba-live-game-card .title {
line-height: 1.2;
        }nba-live-game-card .version {
line-height:1;
          color: var(--secondary-text-color);
          border: 1px solid rgba(255,255,255,0.12);
          border-radius: 999px;
          padding: 2px 6px;
        }nba-live-game-card .pill {
border-radius: 999px;
          padding: 4px 8px;
          background: transparent;
        }nba-live-game-card .pill.live { color: var(--success-color); }nba-live-game-card .pill.delayed { color: var(--warning-color); }nba-live-game-card .pill.final { color: var(--primary-text-color); }nba-live-game-card .pill.postponed { color: var(--warning-color); }nba-live-game-card .pill.next { color: var(--secondary-text-color); }nba-live-game-card .pill.idle { color: var(--secondary-text-color); }nba-live-game-card .status {
          color: var(--secondary-text-color);
line-height: 1.25;
          margin-bottom: 10px;
        }nba-live-game-card .scoreboard {
          display: grid;
          gap: 0;
        }nba-live-game-card .scoreboard-rich {
          grid-template-columns: minmax(0, 1fr);
        }nba-live-game-card .team-row {
          display: flex;
          align-items: center;
          justify-content: space-between;
          gap: 10px;
          ...
          border-top: none;
          opacity: 0.9;
}nba-live-game-card .team-row.winner {
          opacity: 1;
        }nba-live-game-card .team-row.winner .name,
nba-live-game-card .team-row.winner .score {
}nba-live-game-card .team-row:first-child {
          border-top: none;
        }nba-live-game-card .team-left {
          display: flex;
          align-items: center;
          gap: 10px;
          min-width: 0;
        }nba-live-game-card .logo {
          width: 28px;
          height: 28px;
          object-fit: contain;
          flex: 0 0 28px;
        }nba-live-game-card .logo.placeholder {
          border-radius: 50%;
          background: rgba(255,255,255,0.08);
        }nba-live-game-card .player-shot {
        ...
        }nba-live-game-card .player-shot.placeholder {
          background: rgba(255,255,255,0.08);
        }nba-live-game-card .meta {
          min-width: 0;
        }nba-live-game-card .name {
          line-height: 1.15;
          white-space: nowrap;
          overflow: hidden;
          ...
          font-weight: 500;
        }nba-live-game-card .record {
color: var(--secondary-text-color);
          line-height: 1.15;
        }nba-live-game-card .record-inline {
color: var(--primary-text-color);
          opacity: 0.92;
          font-size: 0.9em;
          font-weight: 400;
}nba-live-game-card .team-right {
          display:flex;
          align-items:center;
          gap:6px;
          margin-left:auto;
        }nba-live-game-card .rhe-header {
          display:flex;
          align-items:center;
          justify-content:space-between;
          margin: 0 0 -1px;
...
line-height: 1;
          padding: 0 0 2px;
        }nba-live-game-card .rhe-spacer {
          flex: 1 1 auto;
        }nba-live-game-card .rhe-cols,
nba-live-game-card .rhe-values {
          display:grid;
          ...
          column-gap: 8px;
          white-space: nowrap;
        }nba-live-game-card .rhe-col {
          text-align:right;
}nba-live-game-card .rhe-col.score {
}nba-live-game-card .rhe-score,
nba-live-game-card .score {
min-width: 20px;
          text-align: right;
          font-variant-numeric: tabular-nums;
          font-size: 1.05em;
          font-weight: 500;
}nba-live-game-card .rhe-num {
color: var(--primary-text-color);
          font-variant-numeric: tabular-nums;
          text-align:right;
          font-size: 1.05em;
          font-weight: 500;
        }nba-live-game-card .scoreboard-main {
          display:grid;
          grid-template-columns:minmax(0,1fr) auto;
          column-gap:6px;
          align-items:start;
        }nba-live-game-card .period-marker-side {
          display:flex;
          align-items:center;
          ...
          min-width:28px;
          padding-top: 0;
        }nba-live-game-card .period-marker-side.has-schedule-nav {
          gap: 2px;
          min-width: 64px;
        }nba-live-game-card .schedule-nav-btn {
          flex: 0 0 auto;
          display: inline-flex;
          align-items: center;
          justify-content: center;
        ...
        }nba-live-game-card .schedule-nav-btn:hover:not([disabled]) {
          color: var(--primary-text-color);
          background: var(--secondary-background-color);
        }nba-live-game-card .schedule-nav-btn:focus-visible {
          outline: 2px solid var(--primary-color);
          outline-offset: 1px;
        }nba-live-game-card .schedule-nav-btn[disabled] {
          opacity: 0.25;
          cursor: default;
        }nba-live-game-card .period-strip {
          display: flex;
          align-items: center;
          justify-content: center;
          gap: 24px;
        ...
        }nba-live-game-card .period-nav-btn {
          flex: 0 0 auto;
          display: inline-flex;
          align-items: center;
          justify-content: center;
          ...
          border-radius: 4px;
          -webkit-tap-highlight-color: transparent;
        }nba-live-game-card .period-nav-btn .tri,
nba-live-game-card .live-expand-strip .tri {
          width: 0;
          height: 0;
          border-left: 6px solid transparent;
          border-right: 6px solid transparent;
        }nba-live-game-card .period-nav-btn .tri-down,
nba-live-game-card .live-expand-strip .tri-down { border-top: 7px solid currentColor; }nba-live-game-card .period-nav-btn .tri-up,
nba-live-game-card .live-expand-strip .tri-up { border-bottom: 7px solid currentColor; }nba-live-game-card .period-nav-btn:hover:not([disabled]) {
          color: var(--primary-text-color);
          background: var(--secondary-background-color);
        }nba-live-game-card .period-nav-btn:focus-visible {
          outline: 2px solid var(--primary-color);
          outline-offset: 1px;
        }nba-live-game-card .period-nav-btn[disabled] {
          opacity: 0.25;
          cursor: default;
        }nba-live-game-card .period-marker-wrap {
          display: flex;
          ...
          min-height: 100%;
          height: 100%;
        }nba-live-game-card .period-marker {
          min-width: 20px;
          text-align: center;
border-radius: 0;
          padding: 0;
          background: none;
          color: var(--secondary-text-color);
        }nba-live-game-card .period-marker.live { color: var(--success-color); }nba-live-game-card .period-marker.history { color: var(--info-color, #4a90d9); }nba-live-game-card .period-marker.delayed { color: var(--warning-color); }nba-live-game-card .period-marker.final { color: var(--primary-text-color); }nba-live-game-card .period-marker.postponed { color: var(--warning-color); }nba-live-game-card .period-stack {
          display:flex;
          flex-direction:column;
          align-items:center;
          justify-content:center;
          line-height:1;
          gap:0px;
        }nba-live-game-card .period-stack .arrow,
nba-live-game-card .period-stack .num,
nba-live-game-card .marker-text {
line-height: 1;
        }nba-live-game-card .period-stack .num {
          font-variant-numeric: tabular-nums;
        }nba-live-game-card .marker-text {
          ...
          min-width: 16px;
          text-align:center;
        }nba-live-game-card .state-panel {
          border-top: 1px solid rgba(255,255,255,0.08);
          margin-top: 10px;
          padding-top: 8px;
color: var(--secondary-text-color);
          text-align: center;
        }nba-live-game-card .delayed-panel { color: var(--warning-color); }nba-live-game-card .final-panel { color: var(--secondary-text-color); }nba-live-game-card .matchup-block {
          border-top: 0;
          margin-top: 6px;
          padding: 6px 0 0;
        }nba-live-game-card .matchup-grid {
          display:grid;
          grid-template-columns: minmax(0,1fr) minmax(0,1fr);
          gap: 24px;
          align-items:start;
        }nba-live-game-card .matchup-grid.enhanced {

          grid-template-columns: minmax(0,1fr) clamp(56px, 14cqi, 100px) minmax(0,1fr);
          column-gap: 0;
          row-gap: 0;
          align-items:start;
        }nba-live-game-card .matchup-grid.enhanced.with-pitch-zone {
          grid-template-columns: minmax(0,1fr) clamp(140px, 42cqi, 260px) minmax(0,1fr);
        }nba-live-game-card .matchup-center {
          display:flex;
          align-items:center;
          justify-content:center;
        }nba-live-game-card .matchup-center.stack-center {
          flex-direction: column;
          gap: 0;
          align-self: start;
          justify-content: flex-start;
        }nba-live-game-card .matchup-center.stack-center > .diamond-center {
          min-height: 0;
        }nba-live-game-card .diamond-center {
          align-self:center;
          min-height: 70px;
        }nba-live-game-card .pitch-zone {
          width: clamp(120px, 36cqi, 240px);
          aspect-ratio: 220 / 190;
          display: flex;
          align-items: center;
          justify-content: center;
        }nba-live-game-card .pitch-zone svg {
          width: 100%;
          height: 100%;
          overflow: visible;
        }nba-live-game-card .pitch-zone-frame {
          fill: rgba(255,255,255,0.04);
          stroke: rgba(255,255,255,0.55);
          stroke-width: 1.5;
        }nba-live-game-card .pitch-zone-plate {
          fill: rgba(255,255,255,0.55);
          stroke: rgba(255,255,255,0.75);
          stroke-width: 1;
        }nba-live-game-card .pitch-zone-dot circle {
          filter: drop-shadow(0 0 1px rgba(0,0,0,0.45));
        }nba-live-game-card .diamond-graphic {
          position: relative;
          ...
          align-items:center;
          justify-content:center;
        }nba-live-game-card .diamond-field {
          position:absolute;
          inset: 12.5%;
        ...
        }nba-live-game-card .diamond-base {
          position:absolute;
          width: 17.9%;
          aspect-ratio: 1 / 1;
          ...
          box-sizing:border-box;
          border-radius: 1px;
        }nba-live-game-card .diamond-base.on {
          background: #63a2ff;
          border-color: rgba(255,255,255,0.70);
          box-shadow: 0 0 0 1px rgba(99,162,255,0.22);
        }nba-live-game-card .diamond-base.home { bottom: 1.8%; left: 50%; margin-left: -8.95%; }nba-live-game-card .diamond-base.first { top: 50%; right: 1.8%; margin-top: -8.95%; }nba-live-game-card .diamond-base.second { top: 1.8%; left: 50%; margin-left: -8.95%; }nba-live-game-card .diamond-base.third { top: 50%; left: 1.8%; margin-top: -8.95%; }nba-live-game-card .diamond-mound {
          position:absolute;
          width: 10.7%;
          aspect-ratio: 1 / 1;
          border-radius: 50%;
        ...
        }nba-live-game-card .matchup-divider {
          width:1px;
          align-self:stretch;
          justify-self:center;
          background: transparent;
        }nba-live-game-card .matchup-side {
          min-width:0;
          display:flex;
          flex-direction:column;
          align-items:center;
          justify-content:flex-start;
          text-align:center;
        }nba-live-game-card .matchup-side.centered {
          text-align:center;
        }nba-live-game-card .matchup-side[data-team-popup] {
          cursor: pointer;
          border-radius: 6px;
        }nba-live-game-card .matchup-side[data-team-popup]:hover {
          background: var(--secondary-background-color, rgba(255,255,255,0.06));
        }nba-live-game-card .matchup-side[data-team-popup]:focus-visible {
          outline: 2px solid var(--warning-color, #ffb300);
          outline-offset: 2px;
        }nba-live-game-card .matchup-copy.centered {
          width:100%;
          text-align:center;
        }nba-live-game-card .matchup-value {
          margin-top: 4px;
line-height:1.15;
          white-space: nowrap;
          overflow: hidden;
        ...
        }nba-live-game-card .player-link {
          color: inherit;
          cursor: pointer;
          text-decoration: none;
        }nba-live-game-card .player-link:hover { text-decoration: underline; }nba-live-game-card .player-link:focus-visible {
          outline: 2px solid var(--warning-color);
          outline-offset: 1px;
          border-radius: 2px;
        }nba-live-game-card .stat-line {
color: var(--primary-text-color) !important;
        }nba-live-game-card .matchup-subtle {
          margin-top: 3px;
line-height:1.16;
          color: var(--secondary-text-color);
          white-space: nowrap;
          overflow: hidden;
          text-overflow: ellipsis;
        }nba-live-game-card .label,
nba-live-game-card .subtle {
          color: var(--secondary-text-color);
        }nba-live-game-card .value {
}nba-live-game-card .subtle {
          margin-top: 4px;
line-height: 1.2;
        }nba-live-game-card .muted-block .value {
          color: var(--secondary-text-color);
}nba-live-game-card .live-panel,
nba-live-game-card .state-panel {
          margin-top: 8px;
          padding-top: 6px;
          border-top: 1px solid rgba(255,255,255,0.08);
        }nba-live-game-card .win-prob-row {
          margin-top: 8px;
          padding-top: 6px;
          border-top: 1px solid rgba(255,255,255,0.08);
        }nba-live-game-card .win-prob-bar {
          position: relative;
          width: 100%;
          height: 20px;
          border-radius: 10px;
          ...
          line-height: 20px;
          font-variant-numeric: tabular-nums;
        }nba-live-game-card .win-prob-fill {
          position: absolute;
          top: 0;
          left: 0;
          bottom: 0;
          background: #1d6bd6;
        }nba-live-game-card .win-prob-label {
          position: absolute;
          top: 0;
          bottom: 0;
          display: inline-flex;
        ...
        }nba-live-game-card .win-prob-label-owner { left: 10px; }nba-live-game-card .win-prob-label-opponent { right: 10px; }nba-live-game-card .live-strip {
          display: grid;
          grid-template-columns: max-content 1fr max-content;
          gap: 10px;
          align-items: center;
        }nba-live-game-card .mini-state {
          display: inline-flex;
          align-items: center;
          justify-content: center;
          min-width: 34px;
          ...
          background: rgba(255,255,255,0.08);
line-height: 1;
        }nba-live-game-card .mini-state.strong {
          color: var(--success-color);
        }nba-live-game-card .mini-state.warning {
          color: var(--warning-color);
        }nba-live-game-card .count-summary,
nba-live-game-card .bases-summary {
line-height: 1.2;
        }nba-live-game-card .count-summary {
          color: var(--primary-text-color);
}nba-live-game-card .bases-summary {
          ...
          text-align: right;
          white-space: nowrap;
        }nba-live-game-card .period-sub {
color: var(--secondary-text-color);
          line-height: 1;
        }nba-live-game-card .count-dots-row {
          margin-top: 2px;
          display: flex;
          align-items: center;
          flex-wrap: wrap;
          gap: 6px 8px;
}nba-live-game-card .on-deck-row {
          display: flex;
          ...
          font-size: 0.85em;
          line-height: 1.15;
        }nba-live-game-card .on-deck-label {
          color: var(--secondary-text-color);
        }nba-live-game-card .on-deck-name {
          color: var(--warning-color);
          font-weight: 500;
        }nba-live-game-card .on-deck-stats {
          color: var(--secondary-text-color);
          font-size: 0.9em;
        }nba-live-game-card .bases-occupancy-row {
          margin-top: 6px;
          padding-top: 2px;
          display:grid;
          ...
          align-items:center;
line-height: 1.15;
        }nba-live-game-card .base-slot {
          white-space: nowrap;
          overflow: hidden;
          text-overflow: ellipsis;
        }nba-live-game-card .base-slot:nth-child(1) { text-align:left; }nba-live-game-card .base-slot:nth-child(2) { text-align:center; }nba-live-game-card .base-slot:nth-child(3) { text-align:right; }nba-live-game-card .base-label {
          color: var(--secondary-text-color);
}nba-live-game-card .base-value {
}nba-live-game-card .base-value.empty-value {
          color: var(--secondary-text-color); }nba-live-game-card .base-value.occupied-value {
color: var(--primary-text-color); }nba-live-game-card .count-dots-row.prominent {
          margin-top: 0;
          padding: 0 0 3px;
          justify-content: center;
          gap: 12px 18px;
        }nba-live-game-card .count-pack {
          display:inline-flex;
          align-items:center;
          gap:6px;
        }nba-live-game-card .dots-label {
          color: var(--secondary-text-color);
        }nba-live-game-card .dots-label.verbose {
color: var(--primary-text-color);
        }nba-live-game-card .count-dots-row.verbose {
          justify-content: space-between;
          gap: 10px;
        }nba-live-game-card .dots {
          display: inline-flex;
          gap: 4px;
          margin-right: 4px;
        }nba-live-game-card .dot {
          width: 7px;
          height: 7px;
          border-radius: 50%;
          background: rgba(255,255,255,0.14);
          display: inline-block;
        }nba-live-game-card .dot.ball.on { background: #4fc3f7; }nba-live-game-card .dot.strike.on { background: #ffb74d; }nba-live-game-card .dot.out.on { background: #ef5350; }nba-live-game-card .muted {
          color: var(--secondary-text-color);
          margin-left: 6px;
        }nba-live-game-card .totals-inline {
          color: var(--secondary-text-color);
          margin-left: 6px;
}nba-live-game-card .plays-panel {
          max-height: 320px;
          overflow-y: auto;
          ...
          flex-direction: column;
          gap: 4px;
        }nba-live-game-card .plays-panel:focus-visible {
          outline: 2px solid var(--warning-color, #ffb300);
          outline-offset: -2px;
        }nba-live-game-card .nba-play-order {
          color: var(--secondary-text-color);
          font-size: .7em;
          text-align: right;
        }nba-live-game-card .play-row {
          display: grid;
          grid-template-columns: minmax(0,1fr) max-content;
          gap: 10px;
          align-items: start;
line-height: 1.2;
}nba-live-game-card .play-text {
          min-width: 0;
          white-space: normal;
          word-break: break-word;
          color: var(--primary-text-color);
        }nba-live-game-card .pitch-row {
          grid-template-columns: minmax(0,1fr);
          margin-top: -2px;
          margin-bottom: -2px;
        }nba-live-game-card .pitch-row .play-text {
          text-align: right;
          ...
          padding-left: 0;
line-height: 1.05;
        }nba-live-game-card .pitch-row .play-indicator { display: none; }nba-live-game-card .pitch-velo-hot {
          color: #ff4d4d !important;
          font-weight: 700;
        }nba-live-game-card .play-indicator {
          color: var(--secondary-text-color);
          white-space: nowrap;
          font-variant-numeric: tabular-nums;
}nba-live-game-card .break-leaders-panel {
          margin-top: 10px;
          padding-top: 0;
          border-top: 0;
        }nba-live-game-card .break-leaders-title {
          text-align: center;
color: var(--secondary-text-color);
          font-weight: 600;
          margin-bottom: 6px;
}nba-live-game-card .break-leaders-grid {
          display: grid;
          grid-template-columns: repeat(3, minmax(0,1fr));
          gap: 10px;
        }nba-live-game-card .break-leaders-card {
          display:flex;
          flex-direction:column;
          align-items:center;
          text-align:center;
          gap:2px;
          min-width:0;
        }nba-live-game-card .break-leaders-name {
line-height: 1.03;
          white-space: nowrap;
          overflow: hidden;
          text-overflow: ellipsis;
          max-width: 100%;
          color: var(--warning-color);
        }nba-live-game-card .break-leaders-stat {
          margin-top: 1px;
          color: var(--secondary-text-color);
          white-space: nowrap;
          overflow: hidden;
          text-overflow: ellipsis;
          max-width: 100%;
        }nba-live-game-card .pregame-panel,
nba-live-game-card .leaders-panel {
          margin-top: 10px;
          padding-top: 8px;
          border-top: 1px solid rgba(255,255,255,0.08);
        }nba-live-game-card .pregame-matchup,
nba-live-game-card .leaders-grid {
          display: grid;
          grid-template-columns: repeat(2, minmax(0, 1fr));
          gap: 10px;
        }nba-live-game-card .featured-player-side,
nba-live-game-card .leaders-col {
          min-width: 0;
        }nba-live-game-card .subtle-inline {
          color: var(--secondary-text-color);
margin-left: 6px;
        }nba-live-game-card .leaders-head {
margin-bottom: 6px;
          color: var(--secondary-text-color);
        }nba-live-game-card .leader-item {
          display: grid;
          grid-template-columns: max-content 1fr max-content;
          gap: 6px;
          align-items: center;
line-height: 1.25;
          margin-top: 3px;
        }nba-live-game-card .leader-cat {
          color: var(--secondary-text-color);
          white-space: nowrap;
        }nba-live-game-card .leader-name {
          min-width: 0;
          overflow: hidden;
          text-overflow: ellipsis;
          white-space: nowrap;
          color: var(--warning-color);
        }nba-live-game-card .leader-val {
white-space: nowrap;
        }nba-live-game-card .linescore {
          margin-top: 10px;
          border-top: 1px solid rgba(255,255,255,0.08);
          padding-top: 8px;
        }nba-live-game-card .linescore-grid {
          display: grid;
          grid-template-columns: max-content repeat(9, minmax(18px, 1fr)) max-content;
          gap: 4px 6px;
          align-items: center;
}nba-live-game-card .period-head,
nba-live-game-card .period-cell,
nba-live-game-card .period-total,
nba-live-game-card .team-abbr {
          text-align: center;
        }nba-live-game-card .period-head,
nba-live-game-card .team-abbr {
          color: var(--secondary-text-color);
        }nba-live-game-card .period-total {
}nba-live-game-card .empty {
          padding: 16px;
          color: var(--secondary-text-color);
        }nba-live-game-card .wrapper.compact-mode {
          padding: 0;
          margin: 0;
        }nba-live-game-card .compact-board {
          display: grid;
          grid-template-columns: minmax(0, 1fr) auto;
          column-gap: 8px;
          align-items: center;
          min-height: 32px;
        }nba-live-game-card .compact-left {
          display: flex;
          flex-direction: column;
          gap: 2px;
          min-width: 0;
        }nba-live-game-card .compact-team-row {
          display:flex;
          align-items:center;
          gap:6px;
          min-width:0;
          padding: 0;
          margin: 0;
        }nba-live-game-card .compact-logo {
          width: 24px;
          height: 24px;
          object-fit: contain;
          flex: 0 0 24px;
          margin-top: 0;
          margin-bottom: 0;
        }nba-live-game-card .compact-name {
line-height: 1.15;
          white-space: nowrap;
          overflow: hidden;
          text-overflow: ellipsis;
          margin: 0;
          padding: 0;
}nba-live-game-card .compact-record {
          color: var(--primary-text-color);
opacity: 0.92;
          margin: 0;
          padding: 0;
}nba-live-game-card .compact-right {
          display:flex;
          align-items:center;
          justify-content:flex-end;
          min-width:58px;
        }nba-live-game-card .compact-next-wrap {
          display:flex;
          flex-direction:column;
          align-items:center;
          gap:0px;
          line-height:1;
        }nba-live-game-card .compact-next-wrap.today-only {
          justify-content:center;
          height:100%;
        }nba-live-game-card .compact-final-wrap {
          display:flex;
          flex-direction:row;
          align-items:center;
          justify-content:flex-end;
          gap:6px;
          line-height:1.05;
        }nba-live-game-card .compact-date,
nba-live-game-card .compact-score {
white-space: nowrap;
          line-height: 1.08;
        }nba-live-game-card .compact-final-scores {
          display:flex;
          flex-direction:column;
          align-items:flex-end;
          justify-content:center;
          gap:1px;
        }nba-live-game-card .compact-next-wrap .compact-date,
nba-live-game-card .compact-next-wrap .compact-time {
          white-space: nowrap;
          line-height: 1.05;
          font-size: 1em !important;
          font-weight: 400 !important;
        }nba-live-game-card .compact-next-wrap.today-only .compact-time {
          font-size: 1em !important;
          font-weight: 500 !important;
        }nba-live-game-card .compact-pill {
color: var(--secondary-text-color);
white-space: nowrap;
          min-width: 12px;
          text-align:center;
        }nba-live-game-card .compact-final-date {
          font-size: 1em;
          font-weight: 400;
        }nba-live-game-card .compact-pill-postponed {
          display:flex;
          align-items:center;
          align-self:center;
          min-height: 34px;
          color: var(--warning-color);
          font-size: 0.85em;
        }nba-live-game-card .compact-final-marker {
          display:flex;
          flex-direction:column;
          align-items:center;
          justify-content:center;
          gap:1px;
          height:100%;
        }nba-live-game-card .compact-final-score-right {
          display:flex;
          align-items:center;
          justify-content:flex-end;
          margin-left:auto;
        }nba-live-game-card .final-score {
          font-size:1.2em;
          font-weight:600;
          min-width:28px;
          text-align:right;
        }nba-live-game-card .compact-mode .team-row.winner .final-score {
          color: var(--primary-color, #03a9f4);
        }nba-live-game-card .live-expandable {
          cursor: pointer;
          outline: none;
          -webkit-tap-highlight-color: transparent;
        }nba-live-game-card .live-expandable:focus-visible {
          box-shadow: 0 0 0 2px var(--primary-color, #03a9f4);
          border-radius: 8px;
        }nba-live-game-card .live-expand-strip {
          display: flex;
          align-items: center;
          justify-content: center;
          height: 11px;
          margin: 2px 0 0;
          color: var(--secondary-text-color);
        }nba-live-game-card .live-expandable.expanded .live-expand-strip {
          height: 9px;
          margin-bottom: -2px;
          opacity: 0.55;
        }nba-live-game-card .live-expandable:hover .live-expand-strip {
          color: var(--primary-text-color);
          opacity: 1;
        }nba-live-game-card .upcoming-expandable {
          cursor: pointer;
          outline: none;
        }nba-live-game-card .upcoming-expandable:focus-visible {
          box-shadow: 0 0 0 2px var(--primary-color, #03a9f4);
          border-radius: 8px;
        }nba-live-game-card .upcoming-details-panel {
          display: flex;
          flex-direction: column;
          gap: 10px;
        ...
        }nba-live-game-card .upcoming-details-panel > :first-child {
          margin-top: 0;
          padding-top: 0;
          border-top: none;
        }nba-live-game-card .upcoming-featured_players-grid {
          display: grid;
          grid-template-columns: 1fr auto 1fr;
          align-items: center;
          gap: 8px;
        }nba-live-game-card .upcoming-featured-player-side {
          display: flex;
          flex-direction: column;
          align-items: center;
          gap: 2px;
          text-align: center;
        }nba-live-game-card .upcoming-featured-player-side.align-right {  }nba-live-game-card .upcoming-featured-player-img {
        ...
        }nba-live-game-card .upcoming-featured-player-img.logo-fallback {
          object-fit: contain;
          padding: 4px;
        }nba-live-game-card .upcoming-featured-player-img.placeholder {
          background: var(--secondary-background-color, rgba(127,127,127,0.15));
        }nba-live-game-card .wrapper.headshot-size-small .player-shot,
nba-live-game-card .wrapper.headshot-size-small .upcoming-featured-player-img {
          width: 40px;
        }nba-live-game-card .wrapper.headshot-size-medium .player-shot,
nba-live-game-card .wrapper.headshot-size-medium .upcoming-featured-player-img {
          width: 56px;
        }nba-live-game-card .wrapper.headshot-size-large .player-shot,
nba-live-game-card .wrapper.headshot-size-large .upcoming-featured-player-img {
          width: 72px;
        }nba-live-game-card .wrapper.headshot-size-xlarge .player-shot,
nba-live-game-card .wrapper.headshot-size-xlarge .upcoming-featured-player-img {
          width: 88px;
        }nba-live-game-card .upcoming-featured-player-name {
          font-weight: 600;
          font-size: 0.92em;
          line-height: 1.1;
          color: var(--warning-color);
        }nba-live-game-card .upcoming-featured-player-stat {
          font-size: 0.85em;
          opacity: 0.9;
        }nba-live-game-card .upcoming-featured-player-stat.secondary {
          opacity: 0.75;
        }nba-live-game-card .upcoming-featured_players-vs {
          font-size: 0.8em;
          opacity: 0.6;
          font-style: italic;
        }nba-live-game-card .panel-heading {
          font-size: 0.78em;
          font-weight: 700;
          ...
          opacity: 0.85;
          margin-bottom: 4px;
        }nba-live-game-card .decisions-panel {
          margin-top: 10px;
          padding-top: 8px;
          border-top: 1px solid rgba(255,255,255,0.08);
        }nba-live-game-card .decisions-grid {
          display: grid;
          grid-template-columns: repeat(auto-fit, minmax(0, 1fr));
          gap: 10px;
          align-items: start;
        }nba-live-game-card .decision-cell {
          display: flex;
          flex-direction: column;
          align-items: center;
          gap: 2px;
          min-width: 0;
          text-align: center;
        }nba-live-game-card .decision-img {
        ...
        }nba-live-game-card .decision-img.placeholder {
          background: rgba(255,255,255,0.06);
        }nba-live-game-card .decision-label {
          font-size: 0.72em;
          font-weight: 600;
          text-transform: uppercase;
          letter-spacing: 0.05em;
          color: var(--secondary-text-color);
        }nba-live-game-card .decision-name {
          font-size: 0.92em;
          line-height: 1.2;
          min-width: 0;
          overflow: hidden;
          text-overflow: ellipsis;
          max-width: 100%;
        }nba-live-game-card .decision-record {
          color: var(--secondary-text-color);
          font-variant-numeric: tabular-nums;
          font-weight: 500;
          margin-left: 2px;
        }nba-live-game-card .decision-stats {
          font-size: 0.8em;
          color: var(--secondary-text-color);
          font-variant-numeric: tabular-nums;
          font-weight: 500;
        ...
        }nba-live-game-card .scoring-plays-panel {
          display: flex;
          flex-direction: column;
          gap: 2px;
        }nba-live-game-card .scoring-play-row {
          display: grid;
          ...
          font-size: 0.86em;
          line-height: 1.3;
        }nba-live-game-card .scoring-play-period {
          color: var(--secondary-text-color);
          font-variant-numeric: tabular-nums;
          font-weight: 600;
        }nba-live-game-card .scoring-play-team {
          color: var(--secondary-text-color);
          font-weight: 600;
          letter-spacing: 0.02em;
        }nba-live-game-card .scoring-play-text {

          min-width: 0;
          white-space: normal;
          overflow-wrap: anywhere;
        }nba-live-game-card .scoring-play-score {
          font-variant-numeric: tabular-nums;
          color: var(--secondary-text-color);
        }nba-live-game-card .leader-empty {
          color: var(--secondary-text-color);
          opacity: 0.6;
        }nba-live-game-card .final-highlights-row {
          display: flex;
          justify-content: center;
          margin: 4px 0 2px;
        }nba-live-game-card .final-highlights-link {
          display: inline-flex;
          align-items: center;
          gap: 8px;
          padding: 6px 12px;
        ...
        }nba-live-game-card .final-highlights-link:hover,
nba-live-game-card .final-highlights-link:focus {
          background: rgba(255, 255, 255, 0.12);
          text-decoration: none;
          outline: none;
        }nba-live-game-card .final-highlights-icon {
          color: var(--primary-color, #03a9f4);
          font-size: 0.85em;
        }nba-live-game-card .final-highlights-label {
          font-weight: 500;
        }nba-live-game-card .upcoming-standings {
          display: flex;
          flex-direction: column;
          gap: 2px;
        }nba-live-game-card .standings-heading {
          font-size: 0.95em;
          font-weight: 600;
          text-align: center;
          opacity: 0.85;
          margin-bottom: 4px;
        }nba-live-game-card .standings-row {
          display: grid;
          ...
          line-height: 1.25;
          border-radius: 4px;
        }nba-live-game-card .standings-row.standings-header {
          font-size: 0.75em;
          font-weight: 600;
          opacity: 0.65;
          text-transform: uppercase;
          letter-spacing: 0.04em;
        }nba-live-game-card .standings-row.my-team {
          background: var(--primary-color, #03a9f4);
          color: var(--text-primary-color, #fff);
          font-weight: 600;
        }nba-live-game-card .standings-name {
          white-space: nowrap;
          overflow: hidden;
          text-overflow: ellipsis;
        }nba-live-game-card .standings-wl {
          text-align: right;
          font-variant-numeric: tabular-nums;
        }nba-live-game-card .standings-gb {
          text-align: right;
          font-variant-numeric: tabular-nums;
        }
nba-live-game-card .nba-score-values { display: grid; grid-template-columns: 14px 42px; column-gap: 7px; }
nba-live-game-card .nba-possession-mark { color: var(--warning-color, #ffb300); font-size: 0.66em; font-weight: 700; }
nba-live-game-card .nba-possession-space { width: 14px; }
nba-live-game-card .nba-period-stack { gap: 4px; min-width: 40px; }
nba-live-game-card .nba-clock { font-size: 0.78em; line-height: 1; white-space: nowrap; font-variant-numeric: tabular-nums; }
nba-live-game-card .nba-situation { font-size: 0.82em; gap: 6px; flex-wrap: wrap; }
nba-live-game-card .matchup-grid.enhanced.nba-matchup { grid-template-columns: minmax(0,1fr) clamp(70px,24cqi,140px) minmax(0,1fr); align-items: center; column-gap: 6px; }
nba-live-game-card .matchup-grid.enhanced.nba-matchup.nba-no-court { grid-template-columns: repeat(2,minmax(0,1fr)); }
nba-live-game-card .nba-court-center { width: 100%; min-width: 0; align-self: center; }
nba-live-game-card .nba-court-center svg, nba-live-game-card .nba-shot-chart svg { display: block; width: 100%; height: auto; }
nba-live-game-card .nba-court-outline { fill: rgba(190,135,65,.12); stroke: var(--secondary-text-color,#9b9b9b); stroke-width: .6; }
nba-live-game-card .nba-court-line { fill: none; stroke: var(--secondary-text-color,#9b9b9b); stroke-width: .45; }
nba-live-game-card .nba-court-paint { fill: rgba(205,120,50,.14); stroke: var(--secondary-text-color,#9b9b9b); stroke-width: .45; }
nba-live-game-card .nba-court-hoop { fill: none; stroke: #de864a; stroke-width: .6; }
nba-live-game-card .nba-court-play { fill: none; stroke: #63a2ff; stroke-width: .5; opacity: .9; }
nba-live-game-card .nba-court-play.home { stroke: #e89554; }
nba-live-game-card .nba-court-play.made { fill: #63a2ff; }
nba-live-game-card .nba-court-play.home.made { fill: #e89554; }
nba-live-game-card .nba-shot-away { color: #63a2ff; }
nba-live-game-card .nba-shot-home { color: #e89554; }
nba-live-game-card .nba-court-label { fill: var(--secondary-text-color,#9b9b9b); font-size: 5px; font-weight: 600; }
nba-live-game-card .nba-court-only { max-width: 160px; margin: 12px auto; }
nba-live-game-card .nba-shot-chart { margin: 10px 0 0; padding: 8px 0 0; border-top: 1px solid var(--divider-color,rgba(127,127,127,.2)); }
nba-live-game-card .nba-chart-caption { text-align: center; color: var(--secondary-text-color); font-size: .7em; margin-top: 2px; }
nba-live-game-card .nba-featured-player-context, nba-live-game-card .nba-leader-category { font-size: .72em; color: var(--secondary-text-color); margin-top: 3px; }
nba-live-game-card .nba-period-summary { white-space: normal; flex-wrap: wrap; text-align: right; }
nba-live-game-card .nba-possession { grid-template-columns: minmax(0,1fr) max-content minmax(0,1fr); column-gap: 6px; font-size: .82em; }
nba-live-game-card .nba-possession .base-slot { display: flex; align-items: center; flex-wrap: wrap; gap: 4px; }
nba-live-game-card .nba-possession .base-slot:last-child { justify-content: flex-end; }
nba-live-game-card .nba-possession-label { text-align: center; color: var(--secondary-text-color); }
nba-live-game-card .nba-scoring-list { max-height: 320px; overflow-y: auto; overscroll-behavior: contain; }
nba-live-game-card .nba-scoring-list:focus-visible { outline: 2px solid var(--warning-color,#ffb300); outline-offset: -2px; }
nba-live-game-card .nba-dnp { color: var(--secondary-text-color); font-size: .8em; font-weight: normal; }
nba-live-game-card .nba-play-clock { color: var(--secondary-text-color); font-size: .75em; white-space: nowrap; margin-right: 4px; }
nba-live-game-card .nba-history-label { text-align: center; color: var(--info-color,#4a90d9); font-size: .85em; margin-bottom: 6px; }
nba-live-game-card .standings-row { grid-template-columns: minmax(0,1fr) 65px 48px; }
nba-live-game-card .leader-item { grid-template-columns: max-content minmax(0,1fr); gap: 1px 6px; }
nba-live-game-card .leader-val { grid-column: 1 / -1; white-space: normal; overflow-wrap: anywhere; }
nba-live-game-card .matchup-copy.centered-copy { width: 100%; max-width: 100%; min-width: 0; }
nba-live-game-card .matchup-side { width: 100%; max-width: 100%; }
nba-live-game-card .matchup-side .stat-line { display: flex; flex-wrap: wrap; justify-content: center; gap: 2px 0; max-width: 100%; white-space: normal; overflow-wrap: normal; text-overflow: clip; }
nba-live-game-card .nba-stat-token { display: inline-block; max-width: 100%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
nba-live-game-card .nba-stat-separator { margin-left: .3em; }
...
if (!customElements.get(CARD_TAG)) customElements.define(CARD_TAG, NbaLiveGameCard);
