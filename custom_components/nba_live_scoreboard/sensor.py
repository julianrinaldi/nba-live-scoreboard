...
async def async_setup_entry(
    ...
    async_add_entities([NbaLiveScoreboardSensor(coordinator, entry)])
...
def build_state_attributes(data: Any) -> dict[str, Any]:
    ...
    ``nba_live_scoreboard/game_at_offset`` WebSocket command (schedule
        ...
        recent_plays.append(
            {
                ...
                "is_foul": play.get("is_foul"),
                "is_turnover": play.get("is_turnover"),
                "is_shot": play.get("is_shot"),
                "is_free_throw": play.get("is_free_throw"),
                "coordinate": play.get("coordinate"),
                "shot_type": play.get("shot_type"),
                "points_attempted": play.get("points_attempted"),
            }
        )
    return {
        ...
        "featured_players": data.featured_players,
...
class NbaLiveScoreboardSensor(CoordinatorEntity[RuntimeData], SensorEntity):
    ...
    def __init__(self, coordinator: RuntimeData, entry: ConfigEntry) -> None:
        ...
        team = str(entry.data.get("team", "nba")).lower()
        self._attr_unique_id = f"{entry.entry_id}_scoreboard"
        self._attr_name = f"NBA Live Scoreboard {team.upper()}"
        # Entity ID will be: sensor.nba_live_scoreboard_ny (for NY team).
        self._attr_suggested_object_id = f"nba_live_scoreboard_{team}"
    ...
    def device_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "name": f"NBA Live Scoreboard {data.team_abbr}",
            "manufacturer": "ESPN / Custom",
            "model": "NBA Live Scoreboard",
